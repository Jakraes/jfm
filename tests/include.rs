...
fn make_test_root() -> Value {
    ...
    root_obj.insert("multiplier".to_string(), Value::Number(10.0));
...
fn test_include_with_aggregation() {
    ...
    if let Value::Object(obj) = result {
        ...
        if let Some(Value::Number(avg)) = borrowed.get("average") {
            assert_eq!(*avg, 30.0, "Average age should be 30");
        } else {
            panic!("Expected average field");
        }
        if let Some(Value::Number(total)) = borrowed.get("total") {
