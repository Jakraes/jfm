...
## New v2 Syntax (JFM v2)

JFM v2 introduces more explicit and powerful syntax for common operations.

### Explicit Pipe Operators

```jfm
// |? - Filter (explicit)
users |? .age > 25                // Filter users where age > 25
users |? .active == true          // Filter active users

// |> - Map (explicit)
users |> .name                    // Map to names
users |> { name: .name, age: .age * 2 }  // Transform each item

// |= - Mutate (add/modify fields)
users |= { senior: .age > 50 }    // Add 'senior' field to each
users |= { fullName: `${.first} ${.last}` }

// |& - Aggregate (reduce to single value)
// (Use with aggregate expressions)

// |# - Tap (side effects, return original)
users |# print("Processing...") |> .name  // Print then continue
```

### Colon Method Chaining

Chain operations fluently with `:method()` syntax:
...
users:where(.active == true):select(.name)
...
Extract fields from nested structures at any depth:

```jfm
// Get all 'value' fields at any nesting level
data.nested..value

// Works with field access chains
root.outer..id

// Use parentheses for simple variables
(data)..value
```

Note: `..` for deep extraction only works when the left side is clearly an object-like expression (field access, method call, etc.). For simple variables, use parentheses: `(myVar)..field`.
...
Create objects by extracting fields directly:

```jfm
// Simple projection
user | { .name, .age }           // { name: "Bob", age: 30 }

// Projection with rename
user | { userName: .name }       // { userName: "Bob" }

// Mixed
user | { .name, years: .age }    // { name: "Bob", years: 30 }
```
